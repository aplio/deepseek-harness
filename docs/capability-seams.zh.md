...
| `ctx.workspaceGit` | `core` | [`api-workspace-git`](../packages/api/workspace-git) | - | - | - | 回答 Session 工作区目录位于哪个 checkout——分支或 detached commit、worktree 目录名，以及 origin 指向的 GitHub 仓库——读取不做缓存，并把拿不到的事实视为一种答案；另外回答正在运行的安装 checkout 是否落后于其 upstream remote 所指的 fork 源，该读取会访问网络，因此按部署间隔缓存。 |
