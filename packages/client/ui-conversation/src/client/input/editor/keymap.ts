 ...
 * falls through to those defaults (Enter's and Shift+Enter's line break,
...
export interface ComposerKeymapHandlers {
  ...
  dismissPopup(): void
  /** Whether the submit chord may submit right now (locked/busy states refuse). */
  canSubmit(): boolean
  /** The Cmd/Ctrl+Enter gesture after every guard passed. */
  submit(): void
...
export function registerComposerKeymap(editor: LexicalEditor, handlers: ComposerKeymapHandlers): () => void {
  ...
  return mergeRegister(
    ...
    editor.registerCommand(KEY_ENTER_COMMAND, (event) => {
      ...
      // no-highlight menu passes down to this keymap's own gesture.
      if (handlers.arbitrate('enter', false) !== 'pass') {
        event?.preventDefault()
        return true
      }
      // Plain Enter is a line break, exactly like Shift+Enter: the composer
      // submits only on the Cmd/Ctrl chord, so a multi-line draft needs no
      // modifier and an IME's closing Enter can never send the draft.
      if (event?.ctrlKey !== true && event?.metaKey !== true) return false
      event.preventDefault()
      if (event.repeat) return true // held-down chord must not machine-gun sends
